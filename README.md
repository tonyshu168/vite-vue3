...
# 将现有vite2 + Vue3项目添加typescript

* 安装typescript, 之前需要全局安装typescript, 如已全局安装过typescript, 则不用再全局安装。
```sh
yarn add typescript -D 或 npm i typescript -D 
```

* 初始化typescript配置, 生成tsconfig.json配置文件
```sh
tsc init
```

* 设置tsconfig.json配置，具体的配置说明可以typescript官网上查看。
```json
{
  "compilerOptions": {
...
* 将main.js改为main.ts, 其它的js可以改成ts文件，也可不用修改，照样可以使用。
```ts
import { createApp } from 'vue'
import App from './App.vue'
import router from './router';
